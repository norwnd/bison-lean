...
func newLink() *tLink {
	...
	cl := newWSClient(ipk.String(), conn, true, func(*msgjson.Message) *msgjson.Error { return nil }, dex.StdOutLogger("ws_TEST", dex.LevelTrace))
	return &tLink{
		cl:   cl,
		conn: conn,
	}
...
func TestClientMap(t *testing.T) {
	...
	go func() {
		...
		srv.connect(ctx, conn, ipk.String(), true)
		wg.Done()
	}()
