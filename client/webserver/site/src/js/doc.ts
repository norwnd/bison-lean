...
/*
 * formatSigFigsWithFormatters formats a number using a "significant figures"
 * formatter when the value is small (< 1000), and falls back to a higher
 * precision formatter when the value's fractional part exceeds maxDecimals.
 * This is upstream's helper used by Doc.formatFourSigFigs; preserved here so
 * upstream call sites that use formatFourSigFigs keep their original behavior
 * alongside dev2's formatBestWeCan.
 */
...
export default class Doc {
   ...
   * representation in conventional units. If the value happens to be an
   * integer, no decimals are displayed. Trailing zeros may be truncated.
   * This preserves upstream's original formatter (minimum 2 fractional
   * digits via decimalFormatter) so upstream call sites keep their
   * historical formatting; dev2's formatCoinAtom uses fullPrecisionFormatter
   * with no minimum.
   */
  ...
  /*
   * formatRateFullPrecision is upstream's rate formatter that displays the
   * value with as many fractional digits as the rate step requires (no
   * side-aware rounding). Preserved alongside dev2's formatRateAtomToRateStep
   * so upstream call sites that don't need buyer/seller-side rounding keep
   * working unchanged.
   */
  ...
  /*
   * formatFourSigFigs is upstream's "four significant figures" formatter.
   * For values >= 1000 it shows an integer; otherwise it formats with up to 4
   * significant figures, optionally clamping fractional digits to maxDecimals.
   * Preserved alongside dev2's formatBestWeCan so upstream call sites keep
   * their original numeric formatting.
   */
  ...
  */
  ...
  /*
   * timeSince is upstream's name for ageSinceFromMs (without the trimSeconds
   * parameter). Preserved as an alias so upstream call sites keep working
   * alongside dev2's renamed method.
   */
  static timeSince (ms: number): string {
    return Doc.ageSinceFromMs(ms)
  }
