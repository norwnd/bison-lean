...
// exposed by `mmutil.ts`. In the React port the same data lives on the
// Zustand auth store (`useAuthStore`) and behind typed service
...
// - `app().mmStatus` was non-null at render time in vanilla; in lean
//   it is `MarketMakingStatus | null`, so a few call sites add a
//   nullish fallback.
// - `app().bridgeFeesAndLimits(...)` returned `BridgeFeesAndLimits |
//   null`; the lean wrapper returns `{ ok, result }`, which we unwrap
//   inline.
// - `MM.newDepositAddress` in vanilla wrote the fetched address back
//   into `wallet.address` as a cache. We preserve that (harmless)
//   shared-ref mutation so the second call in a bot-edit flow skips
//   the round trip.
...
import { RateEncodingFactor } from '../../../stores/types'
import { useAuthStore } from '../../../stores/useAuthStore'
import {
  mmStatus as mmStatusAPI,
  marketReport as marketReportAPI,
  maxFundingFees,
  estimateSendTxFee,
  newDepositAddress,
  availableBalances,
...
import { toAllocate, toAllocateRunning, allocationResultAmounts } from './AllocationUtil'
import type { AllocationResult } from './AllocationUtil'
...
function calculateQuoteLot (lotSize: number, baseID: number, quoteID: number, rate?: number): number {
  const { assets, fiatRatesMap } = useAuthStore.getState()
  const baseRate = fiatRatesMap[baseID]
  const quoteRate = fiatRatesMap[quoteID]
  const baseAsset = assets[baseID]
  const quoteAsset = assets[quoteID]
  if (!baseAsset || !quoteAsset) return 0
  ...
  if (baseRate && quoteRate) return lotSize * baseRate / quoteRate * qFactor / bFactor
  if (rate) return lotSize * rate / RateEncodingFactor
  return qFactor
}

function currentWalletDefinition (assetID: number): WalletDefinition | null {
  const asset = useAuthStore.getState().assets[assetID]
  if (!asset) return null
  if (asset.token) return asset.token.definition
  if (!asset.wallet || !asset.info) return null
  return asset.info.availablewallets.find((def) => def.type === asset.wallet!.type) ?? null
}
...
function getWalletMultiFundingOptions (assetID: number): OrderOption[] | null {
  return currentWalletDefinition(assetID)?.multifundingopts ?? null
}
...
async function estimateExternalFee (assetID: number, amount: number): Promise<number> {
  const wallet = useAuthStore.getState().assets[assetID]?.wallet
  if (!wallet || amount <= 0) return 0
  let addr = wallet.address
  if (!addr) {
    const addrRes = await newDepositAddress(assetID)
    if (!addrRes.ok || !addrRes.address) return 0
    addr = addrRes.address
    wallet.address = addr
  }
  const res = await estimateSendTxFee(addr, assetID, amount)
  return res.ok ? res.txfee : 0
}
...
export async function fetchFundingFees (state: BotConfigState): Promise<FundingFeesResult> {
  ...
  try {
    const res = await maxFundingFees(
      market,
      maxBuyPlacements,
      maxSellPlacements,
    ...
    if (!res.ok) return { ok: false, error: res.msg ?? 'maxFundingFees failed', key }
...
export async function fetchRoundTripFeesAndLimits (dexAssetID: number, cexAssetID: number, bridgeName: string): Promise<RoundTripFeesAndLimits> {
  const [withdrawalRes, depositRes] = await Promise.all([
    bridgeFeesAndLimits(dexAssetID, cexAssetID, bridgeName),
    bridgeFeesAndLimits(cexAssetID, dexAssetID, bridgeName)
  ])
  ...
  return { withdrawal: withdrawalRes.result, deposit: depositRes.result, bridgeName, cexAsset: cexAssetID }
}
...
function getMinimumTransferAmounts (
  ...
  const cex = useAuthStore.getState().mmStatus?.cexes[cexName]
...
function getDEXMarketInfo (host: string, baseID: number, quoteID: number): MarketInfo {
  const { assets, walletMap, exchanges } = useAuthStore.getState()
  const baseAsset = assets[baseID]
  const quoteAsset = assets[quoteID]
...
export async function initialBotConfigState (
  ...
  const balancesRes = await availableBalances(
    { host, baseID, quoteID },
    config.cexBaseID,
    config.cexQuoteID,
    config.cexName
  )
  if (!balancesRes.ok) return balancesRes.msg ?? 'Failed to fetch available balances'

  const marketReportRes = await marketReportAPI(host, baseID, quoteID)
  if (!marketReportRes.ok || !marketReportRes.report) {
    return `Failed to get market report: ${marketReportRes.msg ?? 'unknown error'}`
  }
  ...
  const botConfigState: BotConfigState = {
    ...
    availableDEXBalances: balancesRes.dexBalances,
    availableCEXBalances: balancesRes.cexBalances,
    baseBridges,
    quoteBridges,
    baseBridgeFeesAndLimits,
    quoteBridgeFeesAndLimits,
    ...
    runStats: null,
    marketReport: marketReportRes.report,
    ...
    cexStatus,
    fiatRatesMap: useAuthStore.getState().fiatRatesMap,
    buyFundingFees: 0,
    sellFundingFees: 0,
    baseExternalFee: 0,
    quoteExternalFee: 0,
...
export async function botConfigStateFromSavedConfig (
  ...
  const autoRebalance: AutoRebalanceConfig | undefined = savedBotConfig.autoRebalance
    ? {
  ...
  const marketReportRes = await marketReportAPI(savedBotConfig.host, savedBotConfig.baseID, savedBotConfig.quoteID)
  if (!marketReportRes.ok || !marketReportRes.report) {
    return `Failed to get market report: ${marketReportRes.msg ?? 'unknown error'}`
  }

  const statusRes = await mmStatusAPI()
  const botStatus = statusRes.ok && statusRes.status
    ? statusRes.status.bots.find((b: MMBotStatus) =>
  ...
  return config.uiConfig.usingQuickBalance
    ...
    : clampOriginalAllocations(syncedState, balancesRes.dexBalances, balancesRes.cexBalances)
}
...
export const useBotConfigDispatch = (): React.Dispatch<BotConfigAction> => {
