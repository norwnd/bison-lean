// Pure allocation math for the mmsettings quick-balance flows.
...
//
// Vanilla read assets via the global `app()` singleton exposed by
// `registry.ts`. In the React port the same data lives on the
// Zustand auth store, so `useAuthStore.getState().assets` replaces
// the singleton call. Every other piece of logic is transcribed
// line-for-line.
...
function feeReserveAssetID (assetID: number): number {
  ...
  return assets[assetID]?.token?.parentID ?? assetID
}
