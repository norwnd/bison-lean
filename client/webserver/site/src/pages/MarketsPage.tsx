...
function RecentMatchesTable ({
  recentMatches, bui, qui, currentMkt, quoteUnitLabel, baseUnitLabel
}: RecentMatchesTableProps) {
  // MP-43 (recent matches half): 1 Hz re-render so the age column in
  // the recent matches table ticks live. Vanilla L566: the same ticker
  // also touches `page.recentMatchesLiveList` age cells.
  useSecondTicker()
  return (
            ...
            <section className="rightmost-panel pb-3 position-relative">
              ...
              <div className="flex-stretch-column" ref={orderScrollerRef}>
                ...
                {/* Recent Matches — extracted into RecentMatchesTable so
                    its 1 Hz age ticker (MP-43) only re-renders the table,
                    not the whole MarketsPage. */}
                <RecentMatchesTable
                  recentMatches={recentMatches}
                  bui={bui}
                  qui={qui}
                  currentMkt={currentMkt}
                  quoteUnitLabel={quiConv?.unit ?? 'Price'}
                  baseUnitLabel={buiConv?.unit ?? 'Qty'}
                />
